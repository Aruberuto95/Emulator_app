...
pub fn run_until(device: &mut device::Device, deadline: u64, vi_limit: u64) {
    device.cpu.running = true;
    while device.cpu.running
        && device.cpu.cop0.regs[device::cop0::COP0_COUNT_REG as usize] < deadline
        && device.vi.vi_counter < vi_limit
    {
        ...
        if err {
            // Repeated TLB faults must still consume the caller's CPU slice.
