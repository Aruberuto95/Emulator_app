// Keep upstream intrinsic safety blocks visible when comparing engine updates.
#![allow(unused_unsafe)]
...
#[cfg_attr(target_arch = "x86_64", target_feature(enable = "ssse3,sse4.1"))]
...
pub fn execute_vec(device: &mut device::Device, opcode: u32) {
    ...
    // SAFETY: this function and every vector instruction require the same ISA.
    unsafe { device.rsp.cpu.vec_instrs[(opcode & 0x3F) as usize](device, opcode) }
}
