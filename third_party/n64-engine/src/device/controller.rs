...
impl PakHandler {
    pub fn for_type(pak_type: PakType) -> Option<Self> {
        match pak_type {
            PakType::MemPak => Some(Self { read: mempak::read, write: mempak::write, pak_type }),
            PakType::RumblePak => Some(Self { read: rumble::read, write: rumble::write, pak_type }),
            PakType::None | PakType::TransferPak => None,
        }
    }
}
...
pub fn process(device: &mut device::Device, channel: usize) {
    ...
    match cmd {
        ...
        JCMD_CONTROLLER_READ => {
            ...
            let input = ui::input::get(&device.ui, channel);
...
pub fn pak_switch_event(device: &mut device::Device) {
    for channel in &mut device.pif.channels[..4] {
        if channel.change_pak != PakType::None {
            channel.pak_handler = Some(PakHandler {
                read: mempak::read,
                write: mempak::write,
