...
el perfil Rust correspondiente y copia SDL2.dll junto al ejecutable. Cierre la
aplicación antes de reemplazar su ejecutable. La compilación incremental basta;
...
.\build\bin\Release\clothing_app.exe --rom "ruta\juego.nds"
```
...
Los estados se identifican por ROM y ranura. Los estados GBC/GBA anteriores siguen
...
`<nombre>.sav`, y es distinto de un estado rápido. Haga copias de sus partidas
antes de sustituir manualmente archivos o cambiar de ROM.

## Ejecución sin ventana

```powershell
.\build\bin\Release\clothing_app.exe --headless --rom "ruta\juego.gba" --play --ticks 120
